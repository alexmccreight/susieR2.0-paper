...
% -- Ordinary RSS --
\draw[dashbox] (\sL,5.55) rectangle (\sR,6.55);
\node[boxtitle] at (\sCx,6.33) {Ordinary RSS};
\node[codestyle] at (\sCx,6.00) {susie\_rss(z,R,n)};
\node[enhcode] at (\sCx,5.70) {susie\_rss(z,X,n)};
\node[starmark] at (3.48,5.70) {$\bigstar$};

% -- LD Mismatch Correction --
\draw[dashbox] (\sL,3.40) rectangle (\sR,5.20);
\node[boxtitle] at (\sCx,4.97) {LD Mismatch Correction};
\node[enhcode] at (\sCx,4.65) {susie\_rss(z,R,n,};
\node[enhcode] at (\sCx,4.35) {R\_finite=B)};
\node[starmark] at (3.48,4.35) {$\bigstar$};
\node[enhcode] at (\sCx,3.95) {susie\_rss(z,R,n,};
\node[enhcode] at (\sCx,3.65) {R\_mismatch="eb")};
\node[starmark] at (3.48,3.65) {$\bigstar$};

% -- Mixture of LD References --
\draw[dashbox] (\sL,1.10) rectangle (\sR,3.05);
\node[boxtitle] at (\sCx,2.83) {Mixture of LD References};
\node[enhcode] at (\sCx,2.45) {susie\_rss(z,};
\node[enhcode] at (\sCx,2.15) {R=list(R$_1$,\ldots,R$_K$),n)};
\node[starmark] at (3.48,2.15) {$\bigstar$};
\node[enhcode] at (\sCx,1.75) {susie\_rss(z,};
\node[enhcode] at (\sCx,1.45) {X=list(X$_1$,\ldots,X$_K$),n)};
\node[starmark] at (3.48,1.45) {$\bigstar$};
...
\node[codestyle] at (5.95,2.85) {<method>.ss\_mixture()};
...
\draw[dashbox] (\wL,5.40) rectangle (\wR,6.15);
\node[boxtitle] at (\wCx,5.97) {Unmappable Effect Models};

% SuSiE-ash pill
\draw[ashRed, rounded corners=2pt, line width=0.5pt, fill=ashRed!8]
    (8.55,5.50) rectangle (9.90,5.80);
\node[font=\bfseries\scriptsize, text=ashRed] at (9.225,5.65) {SuSiE-ash};
\node[starmark] at (10.08,5.65) {$\bigstar$};

% SuSiE-inf pill
\draw[infGreen, rounded corners=2pt, line width=0.5pt, fill=infGreen!8]
    (10.30,5.50) rectangle (11.65,5.80);
\node[font=\bfseries\scriptsize, text=infGreen] at (10.975,5.65) {SuSiE-inf};
\node[starmark] at (11.85,5.65) {$\bigstar$};

% -- LD Mismatch Correction --
\draw[dashbox] (\wL,4.30) rectangle (\wR,5.25);
\node[boxtitle] at (\wCx,5.03) {LD Mismatch Correction};
\node[enhtext] at (\wCx,4.73) {Finite Reference Panel};
\node[starmark] at (11.85,4.73) {$\bigstar$};
\node[enhtext] at (\wCx,4.43) {Empirical Bayes Mismatch};
\node[starmark] at (11.85,4.43) {$\bigstar$};

% -- SER and Residual Variances --
\draw[dashbox] (\wL,2.95) rectangle (\wR,4.15);
\node[boxtitle] at (\wCx,4.00) {SER and Residual Variances};
\node[font=\scriptsize, text=textMain] at (\wCx,3.70) {Maximum ELBO};
\node[enhtext] at (\wCx,3.40) {Method of Moments};
\node[starmark] at (11.85,3.40) {$\bigstar$};
\node[enhtext] at (\wCx,3.10) {Inverse Gamma Prior};
\node[starmark] at (11.85,3.10) {$\bigstar$};

% -- Multi-Panel LD Weight Update --
\draw[dashbox] (\wL,2.05) rectangle (\wR,2.80);
\node[boxtitle] at (\wCx,2.60) {Multi-Panel LD Weight Update};
\node[font=\itshape\scriptsize, text=enhanced] at (\wCx,2.25) {Mixture Weight Optimization};
\node[starmark] at (11.85,2.25) {$\bigstar$};
...
\pgfmathsetmacro{\sMidA}{(5.55+6.55)/2}
\pgfmathsetmacro{\sMidM}{(3.40+5.20)/2}
\pgfmathsetmacro{\sMidB}{(1.10+3.05)/2}

\draw[busline] (3.75,\iMid) -- (\busX,\iMid);
\draw[busline] (3.75,\sMidA) -- (\busX,\sMidA);
\draw[busline] (3.75,\sMidM) -- (\busX,\sMidM);
