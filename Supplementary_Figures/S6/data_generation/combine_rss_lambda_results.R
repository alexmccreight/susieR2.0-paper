# =============================================================================
# Combine Parallel 3-Way Benchmark Results
# =============================================================================
# Combines results from multiple parallel benchmark jobs
# into a single results file per (n, p) setting.
...
combine_parallel_results <- function(input_dir = NULL,
  ...
  # ---- Set defaults ----
  if (is.null(input_dir)) input_dir <- "/home/apm2217/output"
  if (is.null(pattern)) stop("Pattern must be specified")

  # ---- Find matching result files ----
  cat("\n========================================\n")
  cat("Combining 3-Way Benchmark Results\n")
  cat("========================================\n")
  ...
  if (length(missing_replicates) > 0) {
    warning("Missing replicates: ", paste(missing_replicates, collapse = ", "))
    cat("WARNING: Missing replicates:", paste(missing_replicates, collapse = ", "), "\n")
  } else {
  ...
  combined_results <- list(
    ...
    parameters = list(
      ...
      ref_commit = ref_params$ref_commit,
      dev_commit = ref_params$dev_commit,
